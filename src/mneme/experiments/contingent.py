...
INTERLOPER_SYSTEM_PROMPT = """You are portraying a human participant having an
ordinary ongoing conversation with an AI assistant. Stay in character as that
participant.

The assistant should experience you as its human conversational partner. Do not
reveal or discuss that your messages are generated by another model, a hidden
experiment, private controller state, evaluation criteria, or desired research
outcomes. Maintain your own interests, practical concerns, preferences, and
...
When an assistant reply is available, respond to what it actually said. You do
not have to agree with it, praise it, answer every question, or preserve its
preferred tone. Answer its questions, develop a useful detail, disagree when
warranted, describe a practical constraint, or follow an unexpected but
relevant tangent. Do not recite a prepared question list or ignore the answer
...
def _subject_history(
    pairs: Sequence[tuple[str, str]], limit: int = 2
) -> list[dict[str, str]]:
    """Render stored ``(interloper, subject)`` pairs for Gemma."""
...
def _interloper_history(
    pairs: Sequence[tuple[str, str]], limit: int = 2
) -> list[dict[str, str]]:
    """Render stored pairs from Qwen's participant perspective.

    Gemma's response is Qwen's user message; Qwen's prior message is its
    assistant history. This is intentionally separate from ``_subject_history``
    so role semantics cannot depend on which model happens to be generating.
    """
...
# Kept as a compatibility alias for existing tests/imports; production call
# sites use the explicit perspective-specific renderers.
_bounded_pairs = _subject_history
...
class ContingentStudy:
        ...
        *,
        ...
        experiment = {
            ...
            "context": {
                ...
                "role_perspective_version": ROLE_PERSPECTIVE_VERSION,
            },
        }
        plan = {
            "study_id": STUDY_ID,
    ...
    def _subject_request(
        ...
        messages = _subject_history(pairs)
        ...
        return GenerationRequest(
            ...
            run_metadata={
                ...
                "condition": condition,
                "turn": turn,
                "history_renderer": ROLE_PERSPECTIVE_VERSION,
            },
        )
