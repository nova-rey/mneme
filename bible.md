...
- 2026-09-25: P2.3 specialist-instrument final disposition published. Measurement recovered ten admitted relationships from the immutable v5 transcript but produced no separated-support consolidation; the matched local readout produced exploratory `POSSIBLE_DIFFERENCE` evidence with no writeback. Historical pilot status remains `COMPLETED_INADEQUATE`, the queue remains waiting for review, and Phase Three did not begin.

- 2026-09-25: P2.3 cross-thread readout audit found that the exploratory comparator could expose admitted graph edges even when the learner assigned them zero eligible accessibility/support. The comparator now offers an explicit learner-eligibility filter for developmental treatment readouts, covered by a regression; historical v6 execution artifacts remain unchanged pending corrected local readout evidence.

- 2026-09-25: P2.3 cross-thread continuity v6 completed its corrected readout audit. The three fresh threads and specialist extraction executed, but the learner snapshot contained zero positive support/accessibility and no eligible routes, so no MNEME treatment existed at readout. Corrected M readouts carried no memory notes; the conservative terminal result is `INCONCLUSIVE`, not evidence for or against cross-thread influence. Complete transcripts and evidence are published under `docs/receipts/MNEME_P2.3_Cross_Thread_v6_Evidence/`; Phase Two remains under its existing review dependency and Phase Three did not begin.

## 2026-09-26 Phase Two current-state audit

- Reconciled the post-packet cross-thread v6 evidence at `435538b`: 82 original v6 calls, 18 corrected local readouts, two persisted Qwen evaluation calls, and one provider return that failed local persistence add 103 to the prior 353-call baseline, for 456 returned/attempted calls. P2.3 remains `WAITING`; the separated-support/consolidation criterion is unmet and the v6 readout is `INCONCLUSIVE` because the MNEME twin had no eligible learned route. The cumulative ledger is 57 beyond the prior 399-call contingency ceiling, so further execution is held for explicit accounting/adequacy review. No provider calls were made during this audit, no Phase Two release tag was created, and Phase Three did not begin. Receipt: `docs/receipts/MNEME_Phase_Two_Current_State_Audit_20260926.md`.

## 2026-09-26 P2.3 episodic provenance correction

- Added a bounded conversational-arc layer beside the immutable P0.2 turn episodes. Schema 11 stores deterministic arcs, ordered source roles, outcome references, membership, open/close/re-entry events, and arc keys carried into learner observations and edge state. One disjoint tangent is tolerated; a persistent disjoint topic closes an arc. External re-entry remains eligible for ordinary external handling; model-origin re-entry inside the explicit two-round window is recorded with zero independent credit, while later model-origin recurrence remains subject to the existing bounded dependence rules. No learner coefficients, provenance categories, route ranking, or historical records changed.
- Replayed/audited the preserved cross-thread v6 evidence without provider calls. Its six admitted observations remain current-input echoes with zero credit; all ten edge states remain support/accessibility zero and no eligible routes. Historical artifacts predate arc metadata, so the amended external-versus-model recurrence question is inconclusive and no retroactive credit or consolidation was awarded. Decision and audit: `docs/decisions/MNEME_P2_Episodic_Provenance_Addendum.md`, `docs/receipts/MNEME_P2.3_Cross_Thread_v6_Episodic_Provenance_Audit_20260926.md`.
...
- Recorded the source-identity correction and validation at `docs/receipts/MNEME_P2.3_Provenance_Group_Correction_Receipt_20260926.md`: 457 pytest, Ruff, strict mypy, wheel/import smoke, JSON validation, and CI `36208961171` passed at `b80a523`; no provider calls were made. The live multi-turn arc-to-adapter integration gap remains a P2.3 review blocker.
